...
- 30657f1: Implement the ecash facade (Task T7), providing quote-then-send out-of-band note spending, deterministic redemptions, cancellation handling, and operation log backfilling.
...
- 54adb44: Implement error details envelope codec for length-delimited binary serialization, strict decoding, and forward-compatible error handling across boundaries (Task T3).
...
- c65cc13: Propagate immediate payment failure in `waitForReceive` when invoice receive state is canceled instead of waiting for timeout.
