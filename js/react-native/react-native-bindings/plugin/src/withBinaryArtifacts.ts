...
function hashFileSync(filePath: string): string {
  ...
  try {
    while ((bytesRead = fs.readSync(fd, buf, 0, HASH_CHUNK_SIZE, null)) > 0) {
...
function downloadBinaryArtifacts(): void {
  ...
  try {
    ...
    if (androidChecksum) {
      const actualAndroidChecksum = hashFileSync(
        path.join(packageRoot, 'android-artifacts.zip'),
      )
      ...
      console.log('Android artifacts checksum verified successfully.')
    }
    ...
    fs.rmSync(path.join(packageRoot, 'android-artifacts.zip'), { force: true })
  ...
  try {
    ...
    if (iosChecksum) {
      const actualIosChecksum = hashFileSync(
        path.join(packageRoot, 'ios-artifacts.zip'),
      )
      ...
      console.log('iOS artifacts checksum verified successfully.')
    }
    ...
    fs.rmSync(path.join(packageRoot, 'ios-artifacts.zip'), { force: true })
