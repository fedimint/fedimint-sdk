  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # buildToolsVersions/cmakeVersions cover the same range Fedi's
        # android-nixpkgs-based SDK does (their flake.nix): AGP and various
        # gradle plugins request specific sub-versions of their own choosing
        # at configure time (e.g. react-native-bindings/android/build.gradle,
        # generated by ubrn and pinned to AGP 7.2.1, defaults to build-tools
        # 35.0.0 and cmake 3.22.1 regardless of what the app itself requests)
        # — and since the Nix store is read-only, Gradle's SDK manager can't
        # auto-install a missing one at build time like it would on a normal
        # machine. Listing this many up front is cheaper than discovering
        # each missing version one failed build at a time.
