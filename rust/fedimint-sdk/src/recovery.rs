...
// The UniFFI view of `recover`/`resume_recovery`/`recovery_status` above,
// under their real names but different Rust identifiers: `recover` and
// `resume_recovery` return `RecoveryHandle` (defined below) instead of
// `Recovery`, which embeds a bare `Federation` and a generic
// `Operation<RecoveryState>`, neither of which can cross a UniFFI boundary
// as-is. `recovery_status` is already FFI-safe and is routed through the
// same block purely so all three stay together.
#[cfg(feature = "uniffi")]
...
// The UniFFI view of `Operation<RecoveryState>`: `Operation<S>` is generic
// and UniFFI objects cannot be, so `crate::ffi::ffi_operation!`
// monomorphises a newtype object, forwarding every method to the real
// handle. See that macro's documentation in `ffi.rs`. No `details`:
// a recovery has no fixed facts worth persisting, so `RecoveryState` does
// not implement `DetailedOperationState`.
#[cfg(feature = "uniffi")]
crate::ffi::ffi_operation!(RecoveryOperation, RecoveryOperationUpdates, RecoveryState);

/// The result of [`Sdk::recover`]/[`Sdk::resume_recovery`], with `progress`
/// crossing as [`RecoveryOperation`] and `federation` as an `Arc`, rather
/// than the generic `Operation<RecoveryState>` and bare [`Federation`] the
/// real [`Recovery`] carries — a `Federation` referenced from a record
/// field crosses as `Arc<Federation>` like any other object reference.
#[cfg(feature = "uniffi")]
#[derive(Debug, uniffi::Record)]
pub struct RecoveryHandle {
    /// See [`Recovery::federation`].
    pub federation: std::sync::Arc<Federation>,
    /// See [`Recovery::progress`].
    pub progress: std::sync::Arc<RecoveryOperation>,
}
...
impl From<Recovery> for RecoveryHandle {
    fn from(recovery: Recovery) -> Self {
        Self {
            federation: std::sync::Arc::new(recovery.federation),
            progress: std::sync::Arc::new(recovery.progress.into()),
        }
    }
}
